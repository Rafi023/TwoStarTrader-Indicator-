...
const effectEnd = code.indexOf("}, [isLiveTicking, timeframe, activePosition, settings.soundAlerts, basePrice, mt5Offset]);", effectStart);

if (effectStart !== -1 && effectEnd !== -1) {
    const fullEnd = effectEnd + "}, [isLiveTicking, timeframe, activePosition, settings.soundAlerts, basePrice, mt5Offset]);".length;
    
    const replacement = `  // 1. Background Network Poller: Syncs with live market every 5 seconds
  useEffect(() => {
